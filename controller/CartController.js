    ...
    try {
        ...
        if (!Array.isArray(items) || items.length === 0) {
            return res.status(400).json({
                message:
                    "Invalid request. Items must be an array and cannot be empty.",
            });
        }

        let cart = new Cart({ userId, date: Date.now(), items: [] });
        ...
        for (const item of items) {
            ...
            if (!floral) {
                return res
                    .status(404)
                    .json({ message: `Floral with ID ${floralId} not found` });
            }
...
cartController.get("/", authenToken, async (req, res) => {
    try {
        ...
        const cart = await Cart.find({ userId }).populate("items.floralId");
